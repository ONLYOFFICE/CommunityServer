...
namespace ASC.Web.Studio.HttpHandlers
{
    public class FCKEditorFileUploadHandler : AbstractHttpAsyncHandler
    {
        ...
        private void OldProcessRequest(HttpContext context)
        {
            try
            {
        ...
        private void NewProcessRequest(HttpContext context)
        {
            try
            {
                var editorId = context.Request["CKEditor"];
                var funcNum = context.Request["CKEditorFuncNum"];
                var langCode = context.Request["langCode"];
                var storeDomain = context.Request["esid"];
                var itemID = context.Request["iid"] ?? "";
                ...
                if (file.ContentLength > SetupInfo.MaxImageUploadSize)
                {
                    NewSendFileUploadResponse(context, string.Empty, editorId, funcNum, langCode, FileSizeComment.FileImageSizeExceptionString);
                    return;
                }
                ...
                if (FileUtility.GetFileTypeByFileName(filename) != FileType.Image)
                {
                    NewSendFileUploadResponse(context, string.Empty, editorId, funcNum, langCode, Resource.ErrorUnknownFileImageType);
                    return;
                }
                ...
                NewSendFileUploadResponse(context, saveUri, editorId, funcNum, langCode, string.Empty);
            }
            catch (Exception e)
            {
                NewSendFileUploadResponse(context, string.Empty, string.Empty, string.Empty, string.Empty, e.Message.HtmlEncode());
            }
        }

        private void OldSendFileUploadResponse(HttpContext context, int errorNumber, bool isQuickUpload, string fileUrl, string fileName, string customMsg)
        {
        ...
        private void NewSendFileUploadResponse(HttpContext context, string fileUrl, string editorId, string funcNum, string langCode, string errorMsg)
        {
            context.Response.Clear();
