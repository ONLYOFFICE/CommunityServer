...
window.folderPanel = (function($) {
    ...
    function init() {
        if (isInit === false) {
            ...
            serviceManager.bind(window.Teamlab.events.getMailFolders, _onGetMailFolders);
    ...
    function _showQuestionBox(folderId, questionText) {
    ...
    function _onGetMailFolders(params, folders) {
    ...
    function setTPUnreadMessagesCount(unread_count) {
        $('#TPUnreadMessagesCount').text(unread_count > 100 ? '>100' : unread_count);
        $('#TPUnreadMessagesCount').parent().toggleClass('has-led', unread_count != 0);
    }
    ...
    function decrementUnreadCount(folder_id) {
        ...
        if(folder_id == TMMail.sysfolders.inbox.id)
