    ...
    var _onGetFullTLContacts = function(params, contacts) {
        ...
        for (var i = 0; i < count; i++) {
            if (contacts[i].email != undefined && contacts[i].email != '')
                _teamlab_contacts.push({ 'firstName': contacts[i].firstName, 'lastName': contacts[i].lastName, 'email': contacts[i].email, 'id': contacts[i].id, 'displayName': contacts[i].displayName });
        }
    };
