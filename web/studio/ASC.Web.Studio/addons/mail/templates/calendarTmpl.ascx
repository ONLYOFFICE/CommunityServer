                ...
                <tr>
                    <td class="cal_info">
                        ...
                        <div class="cal_details">
                            <a onclick="${showDetail}" class="link dotline"><%=MailScriptResource.MoreDetailsLabel%></a>
                            {{if typeof(eventUrl) != 'undefined'}}
                            <span style="line-height: 0;padding-right: 8px;">&nbsp;</span>
                            <a href="${eventUrl}" target="_blank" class="link underline"><%=MailScriptResource.GoToEventLink%></a>
                            {{/if}}
                        </div>
                        {{/if}}
                        <table class="cal_card">
                            <tbody>
                                <tr class="card_row card_when">
                                    <td class="card_label"><%=MailScriptResource.CalendarWhenLabel%>:{{if ASC.Mail.Constants.CALENDAR_AVAILABLE === true }}{{tmpl({ "messageId": messageId }, {}) "calendarDateHelperTmpl"}}{{/if}}</td>
                                ...
                                <tr class="card_row">
                                    <td class="card_label"><%=MailScriptResource.CalendarRecurrenceLabel%>:</td>
                                ...
                                <tr class="card_row card_location">
                                    <td class="card_label"><%=MailScriptResource.CalendarWhereLabel%>:</td>
                                    <td class="card_value{{if eventLocationChanged == true }} line-through{{/if}}" {{if typeof(location) != 'undefined' &&  location != ''&& location != null }}title="${location}"{{/if}}>
                                        ...
                                        {{else}}
                                            <%=MailScriptResource.CalendarNoLocationLabel%>
                                        {{/if}}</td>
                                </tr>
                                <tr class="card_row">
                                    <td class="card_label"><%=MailScriptResource.CalendarWhoLabel%>:</td>
                                    <td class="card_value{{if eventOrganizerChanged == true }} line-through{{/if}}">
                                        ...
                                        {{else}}
                                            ${orgName}
                                        {{/if}}&nbsp;(<%=MailScriptResource.CalendarOrganizerLabel%>)
                                    </td>
                                </tr>
                                {{if typeof(calendarName) != 'undefined' && calendarName != '' && calendarName != null }}
                                <tr class="card_row">
                                    <td class="card_label"><%=MailScriptResource.CalendarLabel%>:</td>
                                    <td class="card_value">
                                        ${calendarName}
                                    </td>
                                </tr>
                                ...
                                <tr class="card_row cal_buttons_row" style="display: none;">
                                    <td class="card_label"><%=MailScriptResource.CalendarReplyLabel%>:</td>
                                    <td class="card_value">
                                        <div class="cal_buttons">
                                            <input type="radio" id="request-accept" class="button gray middle" name="calendar-request">
                                            <label for="request-accept"><%=MailScriptResource.CalendarReplyYesLabel%></label>

                                            <input type="radio" id="request-maybe" class="button gray middle" name="calendar-request">
                                            <label for="request-maybe"><%=MailScriptResource.CalendarReplyMaybeLabel%></label>

                                            <input type="radio" id="request-decline" class="button gray middle" name="calendar-request">
                                            <label for="request-decline"><%=MailScriptResource.CalendarReplyNoLabel%></label>
                                        </div>
                                    </td>
                                </tr>
                                {{/if}}
                    ...
                    <td class="cal_agenda">
                        <div class="agenda_label">
                            <p style="margin: 0;"><%=MailScriptResource.CalendarAgendaLabel%></p>
                        </div>
                        <div class="agenda_date">
                            <span>${agendaDate}</span>
                            <span style="line-height: 0;">&nbsp;</span>
                            <a href="/addons/calendar/#agendaDay/${timeDay}" target="_blank" class="link underline"><%=MailScriptResource.GoToCalendarLink%></a>
                        </div>
...
<script id="calendarPopupTmpl" type="text/x-jquery-tmpl">
    <div class="calendarPopView popup">
        <table>
            <tbody>
                <tr>
                    <td class="cal_info">
                        ...
                        <table class="cal_card">
                            <tbody>
                                <tr class="card_row">
                                    <td class="card_label"><%=MailScriptResource.CalendarWhenLabel%>:</td>
                                ...
                                <tr class="card_row card_location">
                                    ...
                                    <td class="card_value" {{if typeof(location) != 'undefined' &&  location != ''&& location != null }}title="${location}"{{/if}}>
                                        {{if typeof(location) != 'undefined' && location != '' && location != null }}
                                             ${location}&nbsp;<a href="${mapUrl}" target="_blank" class="link underline">(<%=MailScriptResource.CalendarMapLabel%>)</a>
                                        {{else}}
                                            <%=MailScriptResource.CalendarNoLocationLabel%>
                                        {{/if}}
                                    </td>
                ...
                {{if attendees.length > 0 }}
                    <tr>
                        <td>
                            <span><%=MailScriptResource.CalendarParticipantsLabel%>:</span>
        ...
        <div class="buttons">
            <button class="button middle gray cancel" type="button"><%: MailScriptResource.CloseBtnLabel %></button>
        </div>
    </div>
