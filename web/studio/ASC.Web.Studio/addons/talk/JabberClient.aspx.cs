...
namespace ASC.Web.Talk
{
    ...
    public partial class JabberClient : MainPage
    {
        ...
        protected string GetJidServerPartOfJid()
        {
            ...
            if (_cfg.FromTeamlabToOnlyOffice == "true" && tenantDomain != null && tenantDomain.EndsWith(_cfg.ToServerInJid))
            {
                int place = tenantDomain.LastIndexOf(_cfg.ToServerInJid);
                if (place >= 0)
                {
                    return tenantDomain.Remove(place, _cfg.ToServerInJid.Length).Insert(place, _cfg.FromServerInJid);
                }
            }
            return tenantDomain;
        }
