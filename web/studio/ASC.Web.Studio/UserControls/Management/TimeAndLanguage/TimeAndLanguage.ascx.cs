    ...
    public partial class TimeAndLanguage : UserControl
    {
        ...
        protected string RenderTimeZoneSelector()
        {
            ...
            foreach (var timeZone in TimeZoneInfo.GetSystemTimeZones().OrderBy(z => z.BaseUtcOffset))
            {
                ...
                if (!displayName.StartsWith("(UTC"))
                {
                    if (timeZone.BaseUtcOffset != TimeSpan.Zero)
                    {
                        displayName = string.Format("(UTC{0}{1}) ", timeZone.BaseUtcOffset < TimeSpan.Zero ? "-" : "+", timeZone.BaseUtcOffset.ToString(@"hh\:mm")) + displayName;
                    }
                    else
                    {
                ...
                sb.AppendFormat("<option " + (timeZone.Equals(_currentTenant.TimeZone) ? "selected" : string.Empty) + " value=\"{0}\">{1}</option>", timeZone.Id, displayName);
            }
            sb.Append("</select>");
            return sb.ToString();
        }
