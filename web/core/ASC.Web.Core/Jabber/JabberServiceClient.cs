        ...
        public void AddXmppConnection(string connectionId, string userName, byte state, int tenantId)
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        service.AddXmppConnection(connectionId, userName, state, tenantId);
                    }
                }
                catch (Exception e)
                {
/*                    Chat.TraceMessage(Chat.TraceError, String.Format("Error on adding of Jabber connection. Username={0}. {1} {2}",
                        userName, e, e.StackTrace));*/
        ...
        public Task SendState(int tenantId, string userName, byte state)
        {
            return Task.Run(() =>
            {
                try
                {
                    ...
                    using (var service = GetService())
                    {
                        service.SendState(tenantId, userName, state);
                    }
                }
                catch (Exception error)
                {
        ...
        {
            Dictionary<string, Dictionary<string, byte>> states = null;
            try
            {
                if (!IsAvailable()) throw new Exception();
                using (var service = GetService())
