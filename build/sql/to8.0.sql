...
ALTER TABLE `crm_organisation_logo`
	CHANGE COLUMN `content` `content` MEDIUMTEXT NOT NULL AFTER `id`;

ALTER TABLE `crm_tag`
	ALTER `title` DROP DEFAULT;
ALTER TABLE `crm_tag`
	CHANGE COLUMN `title` `title` VARCHAR(255) CHARACTER SET 'utf8' COLLATE 'utf8_bin' NOT NULL;
...
ALTER TABLE `files_security`
	CHANGE COLUMN `entry_id` `entry_id` VARCHAR(50) NOT NULL AFTER `tenant_id`,
...
ALTER TABLE `projects_messages`
	ADD COLUMN `status` INT NOT NULL DEFAULT '0' AFTER `title`;
...
INSERT INTO `crm_currency_info` (`resource_key`, `abbreviation`, `symbol`, `culture_name`, `is_convertable`, `is_basic`) VALUES ('Currency_CentralAfricanFranc', 'XAF', 'franc', 'CF', 1, 0);
INSERT INTO `crm_currency_info` (`resource_key`, `abbreviation`, `symbol`, `culture_name`, `is_convertable`, `is_basic`) VALUES ('Currency_WestAfricanFranc', 'XOF', 'franc', 'ML', 1, 0);
INSERT INTO `crm_currency_info` (`resource_key`, `abbreviation`, `symbol`, `culture_name`, `is_convertable`, `is_basic`) VALUES ('Currency_DominicanPeso', 'DOP', 'RD$', 'DO', 1, 0);