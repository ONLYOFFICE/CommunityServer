	...
	class CreatorStartStreamHandler : IXmppStreamStartHandler
	{
		...
		private bool ValidateHost(Jid jid)
		{
			if (jid != null && jid.IsServer)
			{
                ...
                if ((ConfigurationManager.AppSettings["jabber.from-teamlab-to-onlyoffice"] ?? "true") == "true")
                {
                    string fromServerInJid = ConfigurationManager.AppSettings["jabber.from-server-in-jid"] ?? "teamlab.com";
                    string toServerInJid = ConfigurationManager.AppSettings["jabber.to-server-in-jid"] ?? "onlyoffice.com";
                    if (jid.Server.EndsWith(fromServerInJid) &&
                        CoreContext.TenantManager.GetTenant(jid.Server.Replace(fromServerInJid, toServerInJid)) != null)
                    {
                        return true;
                    }
                    if (!jid.Server.EndsWith(toServerInJid) && CoreContext.TenantManager.GetTenant(jid.Server) != null)
                    {
                        return true;
                    }
                }
