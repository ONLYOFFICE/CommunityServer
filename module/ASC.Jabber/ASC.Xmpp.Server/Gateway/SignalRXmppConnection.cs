    ...
    public class SignalRXmppConnection : IXmppConnection
    {
        ...
        public void Send(Node node, Encoding encoding)
        {
            ...
            if (elem != null && elem.To != null)
            {
                ...
                {
                    string nameFrom = elem.From.User;
                    var message = (Message)node;
                    try
                    {
                        if (message.Body != null)
                        {
                            _reverseJabberServiceClient.SendMessage(nameFrom.ToLowerInvariant(), message.To.User.ToLowerInvariant(),
                                message.Body, -1, elem.From.Server);
                        }
                        else if (message.FirstChild.HasTag(typeof(Invite)))
                        {
                            _reverseJabberServiceClient.SendInvite(nameFrom.ToLowerInvariant(),
                                message.To.User.ToLowerInvariant(), elem.To.Server);
                        }
                    }
                    catch (Exception ex)
                    {
