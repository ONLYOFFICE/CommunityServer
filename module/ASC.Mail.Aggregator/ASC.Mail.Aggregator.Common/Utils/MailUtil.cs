...
namespace ASC.Mail.Aggregator.Common.Utils
{
    ...
    {
        public static List<int> GetLabelsFromString(string string_label)
        {
            var list = new List<int>();
            if (!string.IsNullOrEmpty(string_label))
            {
                var labels = string_label.Split(',');
                foreach (var label in labels)
                {
                    int label_in;
                    if (int.TryParse(label, out label_in))
                    {
                        list.Add(label_in);
                    }
                }
