...
namespace ASC.Mail.Aggregator.ComplexOperations
{
    public class MailRecalculateFoldersOperation: MailOperation
    {
        ...
        public MailRecalculateFoldersOperation(Tenant tenant, IAccount user, MailBoxManager mailBoxManager)
            : base(tenant, user)
        {
        ...
        protected override void Do()
        {
            try
            {
                ...
                _mailBoxManager.RecalculateFolders(CurrentTenant.TenantId, CurrentUser.ID.ToString(), progress =>
                {
            ...
            {
                Logger.Error("Mail operation error -> Recalculate folders: {0}", e);
