...
{
    static class MessageHandlersSettings
    {
        public static List<MessageHandlerBase> FromConfig(ILogger log, string connection_string_name)
        {
            var section = (CollectionServiceConfigurationSection)ConfigurationManager.GetSection(Schema.section_name);
            return section.MessageHandlers.GetHandlers(log, connection_string_name);
        }
    }
}
