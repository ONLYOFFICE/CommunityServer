...
{
    class PostfixAdminDbManager
    {
        private readonly PostfixConnectionStringParser _connectionStringParser;
        private readonly int _serverId;

        public PostfixAdminDbManager(int server_id, string connection_string_arg)
        {
            _connectionStringParser = new PostfixConnectionStringParser(connection_string_arg);
            _serverId = server_id;
        }

        public DbManager GetAdminDb()
        {
            var server_db_connection = String.Format("postfixserver{0}", _serverId);
            //Todo: Rewrite with naive implementation of DbRegistry. If we change value in db we can't update it without restarting.
            var cs = new ConnectionStringSettings(server_db_connection,
