...
{
    public class PostfixFactory : MailServerFactoryBase
    {
        public override MailServerBase CreateServer(ServerSetup setup)
        {
            return new PostfixServer(setup);
        }
        ...
        {
            return new PostfixMailGroup(id, tenant, address, in_addresses, server);
        }
        ...
        {
            return new PostfixDomain(id, tenant, name, is_verified, server);
        }

        public override IMailAccount CreateMailAccount(IAccount teamlab_account, string login)
        {
            return new PostfixAccount(teamlab_account, login);
        }
        ...
        {
            return new PostfixDnsSettings(id, tenant, user, domain_name, dkim_selector, dkim_private_key, dkim_public_key, domain_check_name,
