...
namespace ASC.Core.Tenants
{
    public class TenantUtil
    {
        ...
        {
            if (dbDateTime.Kind == DateTimeKind.Utc || dbDateTime == DateTime.MinValue || dbDateTime == DateTime.MaxValue)
            {
                return dbDateTime;
            }

            //Hack
            if (timeZone.IsInvalidTime(DateTime.SpecifyKind(dbDateTime, DateTimeKind.Unspecified)))
            {
                dbDateTime = dbDateTime.AddHours(1);
            }
