...
namespace ASC.SingleSignOn.Saml
{
    public class SamlRequest
    {
        ...
        public string GetRequest(string assertionConsumerServiceUrl, string certPath, string certPassword)
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                try
                {
                    log.DebugFormat("Using certificate for signing saml requests. certPath={0}", certPath);
                ...
                log.DebugFormat("SAML Request: {0}", swString);
                XmlDocument xdoc = new XmlDocument();
                xdoc.LoadXml(swString);
