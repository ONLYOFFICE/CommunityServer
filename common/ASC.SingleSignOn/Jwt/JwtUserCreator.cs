...
namespace ASC.SingleSignOn.Jwt
{
    ...
    {
        private readonly ILog log = LogManager.GetLogger(typeof(JwtUserCreator));
        ...
        public UserInfo CreateUserInfo(ClaimsPrincipal claimsPrincipal, string profile)
        {
            ...
            if (extUserIdClaim != null && !string.IsNullOrWhiteSpace(extUserIdClaim.Value))
            {
                ...
                if (userId != null)
                {
                    log.DebugFormat("Updating new user with extUserId: {0}", extUserIdClaim.Value);
                ...
                {
                    log.DebugFormat("Creating new user with extUserId: {0}", extUserIdClaim.Value);
            ...
            if (!string.IsNullOrEmpty(firstName))
            {
                ...
                {
                    firstName = firstName.Substring(0, MaxNumberOfSymbols);
                }
                userInfo.FirstName = firstName;
            }
            if (!string.IsNullOrEmpty(lastName))
            {
                if (lastName.Length > MaxNumberOfSymbols)
                {
                    lastName = lastName.Substring(0, MaxNumberOfSymbols);
                }
