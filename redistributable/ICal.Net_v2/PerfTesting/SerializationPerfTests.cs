...
";
        ...
        [Benchmark]
        public void Deserialize() => Calendar.LoadFromStream(new StringReader(_sampleEvent))
            .FirstOrDefault()
            .Events
            .FirstOrDefault();

        [Benchmark]
        public void SerializeCalendar() => new CalendarSerializer().SerializeToString(_simpleCalendar);

        private const string _aTzid = "America/New_York";
        private static readonly Calendar _simpleCalendar = new Calendar
        {
            Events = { _e },
        };

        private static readonly Event _e = new Event
        {
